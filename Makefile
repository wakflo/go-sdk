...
cover: ## Displays test coverage in the client and service packages
	...
	go test -coverprofile=cover-connector.out ./connector && go tool cover -html=cover-connector.out
	go test -coverprofile=cover-core.out ./core && go tool cover -html=cover-core.out
